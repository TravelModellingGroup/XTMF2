...
internal sealed class SelectOperator : Expression
{
    /// <summary>
    ...
    public SelectOperator(Expression condition, Expression lhs, Expression rhs, ReadOnlyMemory<char> expression, int offset) : base(expression, offset)
    {
        _condition = condition;
        _lhs = lhs;
        _rhs = rhs;
        TestTypes(_condition, _lhs, _rhs, offset);
    }
    ...
    {
        if(!GetResult(caller, _condition, out var condition, out var errorResult))
        {
            return errorResult;
        }
        if(condition is not bool cond)
        {
            return new ErrorResult("The condition of the expression was not a boolean!", _lhs.Type);
        }
        return (cond ? _lhs : _rhs).GetResult(caller);
    }
    ...
    private static Type[] _SupportedTypes = new[] { typeof(int), typeof(float), typeof(string) };
    ...
    private static void TestTypes(Expression condition, Expression lhs, Expression rhs, int offset)
    {
        if(condition.Type != typeof(bool))
        {
            throw new CompilerException($"The condition of the select (?:) operator must be a boolean!", offset);
        }
        if (lhs.Type != rhs.Type)
        {
            throw new CompilerException($"The True and False sections of select operator are not of the same type! LHS = {lhs.Type.FullName}, RHS = {rhs.Type.FullName}", offset);
        }
        else if (Array.IndexOf(_SupportedTypes, lhs.Type) < 0)
        {
            throw new CompilerException($"The select (?:) operator does not support the type {lhs.Type.FullName}!", offset);
        }
    }
}
