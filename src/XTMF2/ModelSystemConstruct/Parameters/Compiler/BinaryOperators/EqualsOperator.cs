...
/// </summary>
internal sealed class EqualsOperator : Expression
{
    /// <summary>
    ...
    public EqualsOperator(Expression lhs, Expression rhs, ReadOnlyMemory<char> expression, int offset) : base(expression, offset)
    {
        _lhs = lhs;
        _rhs = rhs;
    ...
    internal override Result GetResult(IModule caller)
    {
            ...
            && GetResult(caller, _rhs, out var rhs, out errorResult))
        {
            ...
            else if (lhs is bool lb && rhs is bool rb)
            {
                return new BooleanResult(lb == rb);
            }
            else
            {
    ...
    private static Type[] _SupportedTypes = new[] { typeof(int), typeof(float), typeof(bool) };
