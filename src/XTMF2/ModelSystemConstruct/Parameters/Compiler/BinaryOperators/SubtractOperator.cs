...
/// <summary>
/// This provides the implementation for subtracting
/// </summary>
internal sealed class SubtractOperator : Expression
{
    /// <summary>
    ...
    public SubtractOperator(Expression lhs, Expression rhs, ReadOnlyMemory<char> expression, int offset) : base(expression, offset)
    {
        _lhs = lhs;
        _rhs = rhs;
    ...
    internal override Result GetResult(IModule caller)
    {
            ...
            && GetResult(caller, _rhs, out var rhs, out errorResult))
        {
            ...
            {
                return new ErrorResult($"Unknown type pair for subtraction, {_lhs.Type.FullName} and {_rhs.Type.FullName}!", _lhs.Type);
            }
        }
        return errorResult;
    }
