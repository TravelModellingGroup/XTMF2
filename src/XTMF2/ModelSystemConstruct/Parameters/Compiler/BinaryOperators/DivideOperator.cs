...
/// <summary>
/// This provides the implementation for dividing
/// </summary>
internal sealed class DivideOperator : Expression
{
    /// <summary>
    ...
    public DivideOperator(Expression lhs, Expression rhs, ReadOnlyMemory<char> expression, int offset) : base(expression, offset)
    {
        _lhs = lhs;
        _rhs = rhs;
    ...
    internal override Result GetResult(IModule caller)
    {
            ...
            && GetResult(caller, _rhs, out var rhs, out errorResult))
        {
            ...
            {
                return new ErrorResult($"Unknown type pair for multiplying, {_lhs.Type.FullName} and {_rhs.Type.FullName}!", _lhs.Type);
            }
        }
        return errorResult;
    }
    ...
    private static void TestTypes(Expression lhs, Expression rhs, int offset)
    {
        ...
        {
            throw new CompilerException($"The LHS and RHS of the divide operation are not of the same type! LHS = {lhs.Type.FullName}, RHS = {rhs.Type.FullName}", offset);
        }
