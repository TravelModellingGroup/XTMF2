...
/// <summary>
/// This provides the implementation for exponents
/// </summary>
internal sealed class ExponentOperator : Expression
{
    /// <summary>
    ...
    /// <summary>
    /// Create a new exponent operation.
    /// </summary>
    ...
    internal override Result GetResult(IModule caller)
    {
            ...
            && GetResult(caller, _rhs, out var rhs, out errorResult))
        {
            ...
            {
                return new IntegerResult((int)Math.Pow(l, r));
            }
            else if(lhs is float lf && rhs is float rf)
            {
                return new FloatResult((float)Math.Pow(lf, rf));
            }
            else
            {
