...
/// <summary>
/// View-model wrapper for a <see cref="CalibrationGroup"/>.
/// Exposes the group's name, enabled state, and a synced collection of
/// <see cref="CalibrationEntryViewModel"/> items.
/// </summary>
public sealed partial class CalibrationGroupViewModel : ObservableObject
{
    public CalibrationGroup UnderlyingGroup { get; }

    [ObservableProperty] private string _name;
    [ObservableProperty] private bool   _isEnabled;
    [ObservableProperty] private CalibrationAlgorithmBase _defaultAlgorithm;
    [ObservableProperty] private bool   _isEditing;
    [ObservableProperty] private string _editName = string.Empty;

    /// <summary>Synced entry view-models for this group's parameters.</summary>
    public ObservableCollection<CalibrationEntryViewModel> Parameters { get; } = new();

    private readonly INotifyPropertyChanged   _groupPropNotify;
    private readonly INotifyCollectionChanged _groupParamsNotify;

    public CalibrationGroupViewModel(CalibrationGroup group)
    {
        UnderlyingGroup    = group;
        _name              = group.Name;
        _isEnabled         = group.IsEnabled;
        _defaultAlgorithm  = group.DefaultAlgorithm;
    ...
    private void SyncParameters()
    {
        ...
        foreach (var entry in UnderlyingGroup.Parameters)
            Parameters.Add(new CalibrationEntryViewModel(entry));
    }
    ...
    private void OnGroupPropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        ...
        {
            case nameof(CalibrationGroup.Name):
                Name = UnderlyingGroup.Name;
                break;
            case nameof(CalibrationGroup.IsEnabled):
                IsEnabled = UnderlyingGroup.IsEnabled;
                break;
            case nameof(CalibrationGroup.DefaultAlgorithm):
                DefaultAlgorithm = UnderlyingGroup.DefaultAlgorithm;
                break;
        }
    }
