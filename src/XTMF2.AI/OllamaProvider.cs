            ...
            messages.Insert(0, new
            {
                ...
                content = "For this agent request, return exactly one JSON object with this shape: " +
                    ...
                    "\"id\":\"task-id\",\"title\":\"task title\",\"description\":\"task details\",\"dependsOn\":[],\"actionIds\":[\"stable-action-id\"]}]},\"proposedActions\":[{" +
                            "\"id\":\"stable-action-id\",\"kind\":\"CreateNode, CreateLink, AddLinkDestination, UpdateNode, UpdateParameter, SetBasicParameter, SetScriptedParameter, ConvertBasicParameterToScriptedParameter, CreateCommentBlock, UpdateCommentBlock, or DeleteCommentBlock\",\"summary\":\"what changes\",\"arguments\":{\"id\":\"UUID copied from context ReservedElementIds for the created node or link; existing comment GUID for comment updates/deletes\",\"boundaryId\":\"boundary GUID for CreateNode or CreateCommentBlock\",\"typeName\":\"registered module type for CreateNode\",\"name\":\"node name for CreateNode\",\"comment\":\"comment body for CreateCommentBlock or updated body\",\"header\":\"optional comment header\",\"x\":0,\"y\":0,\"width\":200,\"height\":80,\"originId\":\"origin node GUID for CreateLink or AddLinkDestination\",\"hookName\":\"origin hook name for CreateLink or AddLinkDestination\",\"destinationId\":\"destination node GUID for CreateLink or AddLinkDestination\",\"nodeId\":\"node GUID for parameter updates, or owning module GUID when parameterName is supplied\",\"parameterName\":\"generated parameter hook name when nodeId identifies the owning module\",\"value\":\"literal value for SetBasicParameter or expression for SetScriptedParameter or conversion\",\"isExpression\":false},\"isDestructive\":false}]} . " +
                    ...
                    "Comment-block edits are proposed actions: use CreateCommentBlock with boundaryId, comment, " +
                    "optional header, and positive x, y, width, and height; use UpdateCommentBlock with the exact " +
                    "CommentBlocks[].Id and only the fields that should change; use DeleteCommentBlock with the " +
                    "exact comment ID. Comment IDs are generated by the host for new blocks, so do not use a " +
                    "comment ID as a node or link ID. " +
